...
def remove_nonroad_edges(df: pandas.DataFrame) -> pandas.DataFrame:
    df = df[(df.highway != 'path') & (df.highway != 'footway') & (df.highway != 'steps')]
    df = df[df.service != 'parking_aisle']
    return df


def join_lat_lng(nodes_df: pandas.DataFrame, edges_df: pandas.DataFrame) -> pandas.DataFrame:
    ...
    keep_columns_named = ['from','to','distance','name','highway','service','bridge','tunnel','access','from_lat','from_lng','to_lat','to_lng']
...
def main():
    ...
    parser.add_argument('--nocache', action='store_true', help='Download SRTM data; even if a cache is available on local disk')
    parser.add_argument('where', choices=['kirkland', 'sf'], help='Specify the location to download.')
    ...
    print(f'Gathering dataset from {args.where}. min={ll_min} max={ll_max}. Those are about {distance(ll_min, ll_max)} apart. ')


    if args.nocache or edges_df.empty or nodes_df.empty:
        print('Get network')
        nodes_df, edges_df = osmnet.network_from_bbox(lat_min=ll_min[0], lng_min=ll_min[1], lat_max=ll_max[0], lng_max=ll_max[1])
    ...
    print(f'Eliminated {orig_size-nonroad_removed_size} non-road edges form original total of {orig_size}.')
